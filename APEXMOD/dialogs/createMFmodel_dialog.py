...
from APEXMOD.pyfolder import writeMF
from APEXMOD.pyfolder import db_functions
from APEXMOD.pyfolder import linking_process
...
class createMFmodelDialog(QDialog, FORM_CLASS):
    ...
    # navigate to the DEM raster from APEX
    def loadDEM(self):        
        APEXMOD_path_dict = self.dirs_and_paths()
        settings = QSettings()
        if settings.contains('/APEXMOD/LastInputPath'):
        ...
        if inFileName:
            ...
            layer = QgsRasterLayer(DEM, '{0} ({1})'.format("DEM","APEX"))
            # Put in the group
    ...
    def getElevfromDem(self):
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')
        self.textEdit_mf_log.append(time+' -> ' + "Extracting elevation from APEX DEM ... processing")
        self.label_mf_status.setText("Extracting elevation from APEX DEM ... ")
        self.progressBar_mf_status.setValue(0)
        QCoreApplication.processEvents()
        ...
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')
        self.textEdit_mf_log.append(time+' -> ' + "Extracting elevation from APEX DEM ... passed")
        self.label_mf_status.setText('Step Status: ')
        self.progressBar_mf_status.setValue(100)
        QCoreApplication.processEvents()
